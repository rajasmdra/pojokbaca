...
// ==========================================
if (isset($_POST['simpan_rak'])) {
    $nama_rak = mysqli_real_escape_string($mysqli, trim($_POST['nama_rak']));
    $id_rak   = $_POST['id_rak'] ?? '';

    if (empty($nama_rak)) {
        $_SESSION['gagal_rak'] = "Nama rak tidak boleh kosong!";
    } else {
        if (!empty($id_rak)) {
            // PROSES UPDATE
            $sql_update = "UPDATE rak SET nama_rak = '$nama_rak' WHERE id_rak = '$id_rak'";
            if (mysqli_query($mysqli, $sql_update)) {
                $_SESSION['sukses_rak'] = "Data rak berhasil diperbarui menjadi <strong>$nama_rak</strong>!";
            } else {
                $_SESSION['gagal_rak'] = "Gagal memperbarui data rak.";
            }
        } else {
            // PROSES INSERT
            $sql_insert = "INSERT INTO rak (nama_rak) VALUES ('$nama_rak')";
            if (mysqli_query($mysqli, $sql_insert)) {
                $_SESSION['sukses_rak'] = "Rak <strong>$nama_rak</strong> berhasil ditambahkan!";
            } else {
                $_SESSION['gagal_rak'] = "Gagal menambahkan data rak.";
            }
        }
    }
...
if (isset($_GET['aksi']) && $_GET['aksi'] == 'hapus' && isset($_GET['id'])) {
    ...
    // Cek terlebih dahulu apakah rak sedang digunakan di tabel buku
    $cek_buku = mysqli_query($mysqli, "SELECT id_buku FROM buku WHERE id_rak = '$id_hapus' LIMIT 1");
    if (mysqli_num_rows($cek_buku) > 0) {
        $_SESSION['gagal_rak'] = "Rak gagal dihapus karena masih digunakan oleh data buku.";
    } else {
        $sql_hapus = "DELETE FROM rak WHERE id_rak = '$id_hapus'";
        if (mysqli_query($mysqli, $sql_hapus)) {
            $_SESSION['sukses_rak'] = "Data rak berhasil dihapus!";
        } else {
            $_SESSION['gagal_rak'] = "Gagal menghapus data rak.";
        }
    }
    header("Location: rak.php");
    exit;
...
if (isset($_GET['aksi']) && $_GET['aksi'] == 'edit' && isset($_GET['id'])) {
    $id_edit = $_GET['id'];
    $query_edit = mysqli_query($mysqli, "SELECT * FROM rak WHERE id_rak = '$id_edit'");
    if (mysqli_num_rows($query_edit) > 0) {
        $data_edit = mysqli_fetch_assoc($query_edit);
        $edit_mode = true;
        $val_id_rak = $data_edit['id_rak'];
        $val_nama_rak = $data_edit['nama_rak'];
    }
}
...
$orderby_sql = "nama_rak ASC"; 
if ($sort_by === 'nama') {
    $orderby_sql = (strtoupper($sort_order) === 'DESC') ? "nama_rak DESC" : "nama_rak ASC";
}

$query_tabel = mysqli_query($mysqli, "SELECT * FROM rak ORDER BY $orderby_sql");
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>PojokBaca | Kelola Lokasi Rak</title>
      ...
      <nav class="sidebar-nav">
        ...
        <div class="nav-item-dropdown">
          ...
          <div class="collapse show ms-3" id="menuKelola">
            ...
            <a class="nav-link py-1 small active" href="rak.php"><i class="bi bi-bookshelf me-2"></i>Data Rak</a>
          </div>
        </div>
        ...
        <div class="container-fluid px-3 px-lg-4 py-4">
          <div class="page-heading d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div class="page-heading-copy">
              <span class="page-icon"><i class="bi bi-bookshelf"></i></span>
              <div>
                <p class="eyebrow mb-1">Data Master</p>
                <h1 class="h3 mb-1">Kelola Rak Buku</h1>
                <p class="text-muted mb-0">Atur lokasi penempatan fisik dan nomor rak buku yang tersedia di perpustakaan.</p>
              </div>
            </div>
          </div>

          <?php if (isset($_SESSION['sukses_rak'])): ?>
            <div class="alert alert-success alert-dismissible fade show shadow-sm" role="alert">
              <i class="bi bi-check-circle-fill me-2"></i> <?= $_SESSION['sukses_rak']; ?>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
            <?php unset($_SESSION['sukses_rak']); ?>
          <?php endif; ?>

          <?php if (isset($_SESSION['gagal_rak'])): ?>
            <div class="alert alert-danger alert-dismissible fade show shadow-sm" role="alert">
              <i class="bi bi-exclamation-triangle-fill me-2"></i> <?= $_SESSION['gagal_rak']; ?>
              <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
            <?php unset($_SESSION['gagal_rak']); ?>
          <?php endif; ?>
              ...
              <section class="panel">
                <div class="panel-header">
                  <h2 class="h5 mb-0 section-title">
                    ...
                    <span><?= $edit_mode ? 'Ubah Rak' : 'Tambah Rak Baru'; ?></span>
                  </h2>
                </div>
                <div class="p-3 border-top">
                  <form action="rak.php" method="POST">
                    <input type="hidden" name="id_rak" value="<?= $val_id_rak; ?>">
                    
                    <div class="mb-3">
                      <label for="nama_rak" class="form-label fw-semibold small text-muted">Nama Rak / Lokasi</label>
                      <input type="text" class="form-control" id="nama_rak" name="nama_rak" placeholder="Contoh: R-01, Rak Sains, Lantai 2" value="<?= htmlspecialchars($val_nama_rak); ?>" required autocomplete="off">
                    </div>

                    <div class="d-grid gap-2">
                      <button type="submit" name="simpan_rak" class="btn <?= $edit_mode ? 'btn-warning' : 'btn-primary'; ?> fw-medium">
                        <i class="bi bi-save me-1"></i> <?= $edit_mode ? 'Perbarui Data' : 'Simpan Rak'; ?>
                      </button>
                      <?php if($edit_mode): ?>
                        <a href="rak.php?by=<?= $sort_by; ?>&order=<?= $sort_order; ?>" class="btn btn-light border small fw-medium text-secondary">Batal Edit</a>
                      <?php endif; ?>
                    </div>
                  </form>
                </div>
            ...
            <div class="col-12 col-lg-8">
              <section class="panel">
                <div class="panel-header py-3">
                  <div>
                    <h2 class="h5 mb-1 section-title"><i class="bi bi-table"></i><span>Daftar Rak Buku</span></h2>
                  </div>
                  <input class="form-control form-control-sm table-search" type="search" placeholder="Cari rak..." data-table-search="rakTable" style="max-width: 200px;">
                </div>
                
                <div class="table-responsive">
                  <table class="table align-middle mb-0 table-hover" id="rakTable">
                    <thead>
                      <tr>
                        <th style="width: 80px;">No.</th>
                        <th class="sortable-header">
                          <a href="rak.php?by=nama&order=<?= ($sort_by == 'nama') ? $next_order : 'ASC'; ?>" class="text-decoration-none d-inline-flex align-items-center">
                            Nama Rak Buku <?= getSortIcon('nama', $sort_by, $sort_order); ?>
                          </a>
                        </th>
                    ...
                    <tbody>
                      ...
                      ?>
                        <tr class="<?= ($edit_mode && $val_id_rak == $row['id_rak']) ? 'table-warning opacity-75' : ''; ?>">
                          <td class="text-muted small"><?= $no++; ?>.</td>
                          <td class="fw-semibold d-flex align-items-center gap-2">
                            <span class="brand-icon"><i class="bi bi-bookshelf" aria-hidden="true"></i></span><?= htmlspecialchars($row['nama_rak']); ?>
                          </td>
                          <td class="text-end">
                            <a href="rak.php?id=<?= $row['id_rak']; ?>&aksi=edit&by=<?= $sort_by; ?>&order=<?= $sort_order; ?>" class="btn btn-outline-warning btn-sm px-2 py-1 me-1 fw-medium" title="Ubah Data">
                              <i class="bi bi-pencil me-1"></i>Edit
                            </a>
                            <a href="rak.php?id=<?= $row['id_rak']; ?>&aksi=hapus" class="btn btn-outline-danger btn-sm px-2 py-1 fw-medium" onclick="return confirm('Apakah Anda yakin ingin menghapus rak ini? Tindakan ini akan divalidasi sistem terhadap sirkulasi buku.')" title="Hapus Data">
                      ...
                      ?>
                        <tr>
                          <td colspan="3" class="text-center py-4 text-muted">
                            <i class="bi bi-folder-x display-6 d-block mb-2 text-secondary"></i> Belum ada rekaman rak buku yang tersimpan.
                          </td>
                        </tr>
                      <?php endif; ?>
                    </tbody>
