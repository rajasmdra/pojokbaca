                ...
                <tbody>
                  ...
                  if (mysqli_num_rows($query_buku) > 0): 
                    while($buku = mysqli_fetch_assoc($query_buku)):
                      ...
                      // Fallback pencarian nama kolom database yang dinamis
                      $stok = $buku['stok_tersedia'] ?? $buku['stok_tersedie'] ?? 0; 
                      if($stok > 0) {
                          $status_badge = '<span class="badge text-bg-success">Tersedia (' . $stok . ')</span>';
                      } else {
                          $status_badge = '<span class="badge text-bg-danger">Sedang Kosong</span>';
                      }
                  ?>
                    <tr>
                      <td>
                        <div class="table-media">
                          <span class="brand-icon"><i class="bi bi-book-half" aria-hidden="true"></i></span>
                          <span class="fw-semibold text-truncate" style="max-width: 250px;"><?= htmlspecialchars($buku['judul']); ?></span>
                        </div>
                      </td>
                      <td><?= htmlspecialchars($buku['nama_kategori'] ?? 'Umum'); ?></td>
                      <td>
                      ...
                      <td>
                        <span class="badge bg-light text-dark border">
                          <i class="bi bi-pin-map text-secondary me-1"></i><?= htmlspecialchars($buku['nama_rak'] ?? 'Rak ID: '.$buku['id_rak']); ?>
                        </span>
                      </td>
                      <td><?= $status_badge; ?></td>
                      <td class="text-end">
                        <a href="buku.php?id=<?= $buku['id_buku']; ?>" class="btn btn-light btn-sm fw-medium">
